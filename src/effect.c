...
struct delay_line create_delay_line(size_t nmemb) {
  ...
  if (!buffer) {
    return (struct delay_line){ 0, 0, NULL, 0, 0 };
  }
  memset(buffer, 0, sizeof(f32) * nmemb);
  return (struct delay_line){ 0, nmemb, buffer, 0, 0 };
}
...
struct delay_line increment_delay_line(size_t read_inc, size_t write_inc,
                                       struct delay_line dl) {
  const size_t next_read = (dl.read + read_inc) % dl.end;
  const size_t next_write = (dl.write + write_inc) % dl.end;
  return (struct delay_line){ dl.start, dl.end, dl.buffer, next_read,
                              next_write };
}
