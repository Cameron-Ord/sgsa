...
i32 terminate_controller(void) { return Pm_Terminate(); }

i32 initialize_controller(void) { return Pm_Initialize(); }
...
static struct device_data make_device_data(i32 id, const char *name,
                                           const char *interf, i32 input) {
...
struct midi_input midi_read_input(PortMidiStream *stream, i32 len) {
    ...
    if (count > 0) {
        return (struct midi_input){ Pm_MessageStatus(event.message),
                                    Pm_MessageData1(event.message),
                                    Pm_MessageData2(event.message), true };
    }
    return (struct midi_input){ 0, 0, 0, false };
}
