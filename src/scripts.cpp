...
Literals::Literals(std::vector<Entry> flds, std::vector<std::pair<Entry, std::vector<Entry>>> tbls) 
...
const std::vector<Entry> OSC_TABLE_ENTRIES = {
  Entry("duty_cycle", ENTRY_TYPE::FLOAT), 
  Entry("detune", ENTRY_TYPE::FLOAT), 
  Entry("volume", ENTRY_TYPE::FLOAT), 
...
Cfg_Builder& Cfg_Builder::loop_fields(void){
  ...
  for(size_t i = 0; i < fields.fields_array.size(); i++){
    ...
    switch(lc_.get_type()){
        ...
        case LUA_TNUMBER: {
            if(lc_.is_int()){
              maps.base_cfg.cfg_ints.emplace(fields.fields_array[i].name, lc_.get_int());
              lc_.pop();
            } else {
              maps.base_cfg.cfg_floats.emplace(fields.fields_array[i].name, lc_.get_float());
              lc_.pop();
            }
        }break;

        case LUA_TSTRING:{
            maps.base_cfg.cfg_strings.emplace(fields.fields_array[i].name, lc_.get_str());
            lc_.pop();
        }break;

        case LUA_TBOOLEAN: {
            maps.base_cfg.cfg_bools.emplace(fields.fields_array[i].name, lc_.get_bool());
            lc_.pop();
        } break;
      }
          ...
          for(size_t j = 1; j <= len; j++){
            ...
            for(size_t k = 0; k < fields.tables_array[i].second.size(); k++){
              if(lc_.get_field(fields.tables_array[i].second[k].name.c_str())){
                ...
                if(lc_.is_int()){
                  maps.osc_maps[j - 1].cfg_ints.emplace(fields.tables_array[i].second[k].name, lc_.get_int());
                } else {
                  maps.osc_maps[j - 1].cfg_floats.emplace(fields.tables_array[i].second[k].name, lc_.get_float());
                }
                lc_.pop();
              }
            }
