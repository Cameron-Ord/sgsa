...
void print_config(struct configs config){
    ...
    printf("    VOLUME: (%.3f), SAMPLE GAIN: (%.3f), DELAY GAIN: (%.3f), DELAY FEEDBACK: (%.3f),\n", (f64)ents[MAIN_VOLUME].value, (f64)ents[SAMPLE_GAIN].value, (f64)ents[DELAY_GAIN].value, (f64)ents[DELAY_FEEDBACK].value);
    printf("    VRATE: (%.3f), VDEPTH: (%.3f), VIBRATO ON: (%.3f)\n", (f64)ents[VIBRATO_RATE].value, (f64)ents[VIBRATO_DEPTH].value, (f64)ents[VIBRATO_ONSET].value);
    printf("}\n");
}
