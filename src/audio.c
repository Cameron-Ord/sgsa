...
static void loop_oscilators(struct voice* v, f32 sum[], const struct configs* cfg, f32 dcblock) {
    ...
    for (u32 i = 0; i < v->l.oscilators; i++) {
        ...
        if (state[TIME] > cfg->entries[VIBRATO_ONSET].value) {
            freq = vibrato(cfg->entries[VIBRATO_RATE].value, cfg->entries[VIBRATO_DEPTH].value, freq,
        ...
        case SINE: {
            for (i32 c = 0; c < channels; c++) {
                osc->gen.generated[c] = sine(v->amplitude, state[PHASE]) * spec[OSC_VOLUME].value;
            }
        } break;
        case PULSE_POLY: {
            for (i32 c = 0; c < channels; c++) {
                osc->gen.generated[c] =
                    poly_square(v->amplitude, inc, state[PHASE], spec[COEFF].value) * spec[OSC_VOLUME].value;
            }
        } break;
        case TRIANGLE_POLY: {
            for (i32 c = 0; c < channels; c++) {
                osc->gen.generated[c] =
                    poly_triangle(v->amplitude, inc, state[PHASE], &state[INTEGRATOR], &state[DC_X], &state[DC_Y], dcblock) *
