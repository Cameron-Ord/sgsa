...
static void loop_delay(size_t nsamples, f32 *samples,
                       struct voice_control *vc) {
    for (size_t i = 0; i < nsamples; i++) {
        const f32 delayed =
            delay_line_read(&vc->dl) * vc->cfg.entries[DELAY_FEEDBACK_VAL].value;
        const f32 added =
            tanhf(samples[i] + delayed * vc->cfg.entries[DELAY_GAIN_VAL].value);
...
static void loop_oscilators(struct voice *v, f32 sum[],
                            const struct configs *cfg, f32 dcblock) {
    const i32 channels = (i32)cfg->entries[CHANNELS_VAL].value;
    const i32 samplerate = (i32)cfg->entries[SAMPLE_RATE_VAL].value;
    ...
    for (u32 i = 0; i < v->l.oscilators; i++) {
        ...
        const bool first =
            v->active && env[ENV_STATE_VAL].value != (f32)ENVELOPE_OFF;
        const bool second =
            !v->active && env[ENV_STATE_VAL].value == (f32)ENVELOPE_RELEASE;
        const bool cond = first || second;
        if (!cond) {
            continue;
        }
        f32 freq = v->l.base_freq * spec[OCTAVE_VAL].value * spec[DETUNE_VAL].value;

        if (state[TIME_VAL] > cfg->entries[VIBRATO_ONSET_VAL].value) {
            freq = vibrato(cfg->entries[VIBRATO_RATE_VAL].value,
                           cfg->entries[VIBRATO_DEPTH_VAL].value, freq,
                           cfg->entries[SAMPLE_RATE_VAL].value);
        }
        const f32 inc = freq / cfg->entries[SAMPLE_RATE_VAL].value;
        const f32 dt = 1.0f / cfg->entries[SAMPLE_RATE_VAL].value;
        ...
        case SINE: {
            for (i32 c = 0; c < channels; c++) {
                osc->gen.generated[c] =
                    sine(v->amplitude, state[PHASE_VAL]) * spec[OSC_VOLUME_VAL].value;
            }
        } break;
        case PULSE_POLY: {
            for (i32 c = 0; c < channels; c++) {
                osc->gen.generated[c] = poly_square(v->amplitude, inc, state[PHASE_VAL],
                                                    spec[COEFF_VAL].value) *
                                        spec[OSC_VOLUME_VAL].value;
            }
        } break;
        case TRIANGLE_POLY: {
            for (i32 c = 0; c < channels; c++) {
                osc->gen.generated[c] =
                    poly_triangle(v->amplitude, inc, state[PHASE_VAL],
                                  &state[INTEGRATOR_VAL], &state[DC_X_VAL],
                                  &state[DC_Y_VAL], dcblock) *
                    spec[OSC_VOLUME_VAL].value;
            }
        } break;
        case SAW_POLY: {
            for (i32 c = 0; c < channels; c++) {
                osc->gen.generated[c] = poly_saw(v->amplitude, inc, state[PHASE_VAL]) *
                                        spec[OSC_VOLUME_VAL].value;
            }
        } break;
        case SAW_RAW: {
            for (i32 c = 0; c < channels; c++) {
                osc->gen.generated[c] = sawtooth(v->amplitude, state[PHASE_VAL]) *
                                        spec[OSC_VOLUME_VAL].value;
            }
        } break;
        case TRIANGLE_RAW: {
            for (i32 c = 0; c < channels; c++) {
                osc->gen.generated[c] = triangle(v->amplitude, state[PHASE_VAL]) *
                                        spec[OSC_VOLUME_VAL].value;
            }
        } break;
        case PULSE_RAW: {
            for (i32 c = 0; c < channels; c++) {
                osc->gen.generated[c] =
                    square(v->amplitude, state[PHASE_VAL], spec[COEFF_VAL].value) *
                    spec[OSC_VOLUME_VAL].value;
            }
        } break;
        }
        ...
        f32 *envelope_state = &env[ENV_STATE_VAL].value;
        ...
        for (i32 c = 0; c < channels; c++) {
            ...
            // filter
            osc->gen.filtered_high[c] += linear_interpolate(
                osc->gen.generated[c], osc->gen.filtered_high[c], alpha_high);
            osc->gen.filtered_low[c] += linear_interpolate(
                osc->gen.generated[c], osc->gen.filtered_low[c], alpha_low);
            // mix and sum
            osc->gen.generated[c] = HIGH_MIX * osc->gen.filtered_high[c] +
                                    LOW_MIX * osc->gen.filtered_low[c];
            osc->gen.generated[c] =
                tanhf(osc->gen.generated[c] * (f32)cfg->entries[OSC_GAIN_VAL].value);
            sum[c] += osc->gen.generated[c] / (f32)v->l.oscilators;
        }
        // const f32 out = 0.7 * osc->filtered_low + 0.3 *
        // osc->filtered_high;
        state[PHASE_VAL] += inc;
        if (state[PHASE_VAL] >= 1.0f) {
            state[PHASE_VAL] -= 1.0f;
        }
        state[TIME_VAL] += 1.0f / (f32)samplerate;
    }
    // compress sum
    for (i32 c = 0; c < channels; c++) {
        sum[c] = tanhf(sum[c] * (f32)cfg->entries[SAMPLE_GAIN_VAL].value);
    }
}

static void loop_voicings(struct voice_control *vc,
                          f32 wave_samples[CHANNEL_MAX]) {
    for (i32 c = 0; c < CHANNEL_MAX; c++) {
        wave_samples[c] = 0.0;
    }
    const i32 channels = (i32)vc->cfg.entries[CHANNELS_VAL].value;
...
static void loop_samples(size_t count, f32 *samplebuffer,
                         struct voice_control *vc) {
    const size_t channels = (size_t)vc->cfg.entries[CHANNELS_VAL].value;
    for (size_t n = 0; n < count / channels; n++) {
        f32 wave_samples[CHANNEL_MAX];
        loop_voicings(vc, wave_samples);
        for (size_t c = 0; c < channels; c++) {
            samplebuffer[n * channels + c] =
                wave_samples[c] * vc->cfg.entries[MAIN_VOLUME_VAL].value;
        }
    }
}
