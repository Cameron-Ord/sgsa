...
struct osc_state make_state(void) {
    struct osc_state state = {.entries = {
                                  [PHASE] = ZEROED,
                                  [INTEGRATOR] = ZEROED,
                                  [DC_X] = ZEROED,
                                  [DC_Y] = ZEROED,
...
struct envelope make_env(f32 atk, f32 dec, f32 sus, f32 rel) {
    struct envelope env = {.entries = {
                               [STATE] = {.name = "Envelope state", .name_len = 0, .value = ENVELOPE_OFF},
                               [ENVELOPE] = {.name = "Envelope", .name_len = 0, .value = ZEROED},
                               [ATTACK] =
                                   {
                                       .name = "Attack",
                                       .name_len = 0,
                               ...
                               [DECAY] = {.name = "Decay", .name_len = 0, .value = dec},
                               [SUSTAIN] = {.name = "Sustain", .name_len = 0, .value = sus},
                               [RELEASE] = {.name = "Release", .name_len = 0, .value = rel},
                           }};
...
struct oscilator make_custom_oscilator(i32 wfid, f32 atk, f32 dec, f32 sus, f32 rel, f32 octpos, f32 coeff, f32 vol,
                                       f32 detune) {
    struct oscilator osc = {.waveform_id = wfid,
                            .gen =
                                {
...
struct oscilator make_default_oscilator(i32 wfid) {
    return (struct oscilator){
        ...
        .state = make_state(),
        .spec = make_spec(DEFAULT_OCTAVE_SKIP, DEFAULT_COEFF, DEFAULT_CONTRIBUTION_VOLUME, DEFAULT_DETUNE),
        .env = make_env(DEFAULT_ENV_ATTACK, DEFAULT_ENV_DECAY, DEFAULT_ENV_SUSTAIN, DEFAULT_ENV_RELEASE)};
}

struct wave_spec make_spec(f32 oct, f32 coeff, f32 vol, f32 detune) {
    struct wave_spec spec = {.entries = {
                                 [OCTAVE] = {"Octave increment", .name_len = 0, .value = oct},
                                 [COEFF] = {"Coefficient", .name_len = 0, .value = coeff},
                                 [OSC_VOLUME] = {"Oscilator volume", .name_len = 0, .value = vol},
                                 [DETUNE] = {"Detune", .name_len = 0, .value = detune},
                             }};
