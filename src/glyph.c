...
static void set_glyph(struct glyph *g, i32 width, i32 height, u8 code,
                      SDL_Texture *texture) {
  g->code = code;
  g->width = width;
  g->height = height;
...
static SDL_Surface *render_text_blended(TTF_Font *font, const char *c,
...
void create_glyph_textures(SDL_Renderer *rend, TTF_Font *font,
  ...
  for (unsigned char i = ASCII_FIRST; i < ASCII_SIZE; i++) {
    ...
    SDL_Surface *surface = render_text_blended(
     font, (const char *)chstr, 1, (SDL_Color){ 255, 255, 255, 255 });
    if (!surface) {
      continue;
    }
